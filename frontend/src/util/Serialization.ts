...
export function serializeVertex(cell: Cell): string {
  return JSON.stringify({
    ...
    children: cell.children
      ...
      .map(serializeVertex), // a little bit of recursion :)
  });
}
...
export function serializeGraph(graph: CustomGraph): string {
  return JSON.stringify({
    ...
    edges: graph
      .getChildCells(graph.getDefaultParent())
...
export function deserializeVertex(graph: Graph, serializedCell: string): Cell {
  ...
  const cell = graph.insertVertex({
    id: cellData.cellId,
    parent:
      graph.getDataModel().getCell(cellData.parent) ?? graph.getDefaultParent(),
  ...
  if (cellData.style.shape)
    setupCellForShape(
      cell,
      graph.getCellRenderer().getShape(cellData.style.shape)
    );
...
export function deserializeEdge(graph: Graph, serializedEdge: string): Cell {
  ...
  const edge = graph.insertEdge({
    ...
    parent:
      graph.getDataModel().getCell(edgeData.parent) ?? graph.getDefaultParent(),
